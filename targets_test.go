...
// A --readonly instance must refuse target writes from an ADMIN, not just a
// viewer. The role check alone would let the admin straight through, which is
// exactly the difference between the two mechanisms: the flag is a property of
// the process, so no credential gets past it.
func TestReadOnlyInstanceRefusesAdminWrites(t *testing.T) {
	st, _ := NewStore(t.TempDir(), nil)
	defer st.Close()
	run := NewRunner(ProbeCfg{Packets: 1, TimeoutMs: 50}, st, NewDetector(st))
	h := newMux(&Config{ReadOnly: true}, st, run)

	post := func(path, payload, cookie string) *httptest.ResponseRecorder {
		req := httptest.NewRequest("POST", "http://pp.local"+path, strings.NewReader(payload))
		req.Header.Set("Content-Type", "application/json")
		req.Header.Set("Origin", "http://pp.local")
		if cookie != "" {
			req.Header.Set("Cookie", sessionCookie+"="+cookie)
		}
		w := httptest.NewRecorder()
	...
	// Setup still has to work: a read-only instance must still be reachable, or
	// nobody can look at the graphs it exists to show.
	w := post("/api/setup", `{"User":"admin","Pass":"correct horse battery"}`, "")
	if w.Code != http.StatusOK {
		t.Fatalf("setup on a --readonly instance failed: %d %s", w.Code, w.Body)
	}
	var tok string
	for _, c := range w.Result().Cookies() {
		if c.Name == sessionCookie {
	...
	w = post("/api/targets", `{"name":"x","type":"icmp","host":"127.0.0.1"}`, tok)
	if w.Code != http.StatusForbidden {
		t.Fatalf("admin write on a --readonly instance: got %d, want 403", w.Code)
	}
	if !strings.Contains(w.Body.String(), "--readonly") {
		t.Fatalf("the refusal should name the flag so the operator knows where to look: %s", w.Body.String())
	}
}
