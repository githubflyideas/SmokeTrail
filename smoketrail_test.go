...
// The credential is the one piece of state a service cannot keep on its command
// line, so the hashing has to be right: salted, slow, and constant-time.
func TestPasswordHashRoundTrip(t *testing.T) {
	...
	if h1 == h2 {
		t.Fatal("same password hashed twice must differ — the salt is not random")
	}
	if strings.Contains(h1, pw) {
		t.Fatal("the hash record contains the plaintext")
	}
	if !verifyPassword(h1, pw) {
		t.Fatal("correct password rejected")
	}
	for _, bad := range []string{"", "wrong", pw + " ", " " + pw, strings.ToUpper(pw)} {
		if verifyPassword(h1, bad) {
			t.Fatalf("accepted wrong password %q", bad)
		}
	}
	for _, junk := range []string{"", "notahash", "pbkdf2$sha256$1$a$b", "pbkdf2$md5$600000$a$b"} {
		if verifyPassword(junk, pw) {
			t.Fatalf("accepted malformed record %q", junk)
		}
	}
}

// Portable mode is decided by one filesystem check; getting it wrong would move a
// user's history without telling them.
func TestDataDirRules(t *testing.T) {
	if dir, portable := resolveDataDir("/explicit/path"); dir != "/explicit/path" || portable {
		t.Fatalf("--data must win outright, got %q portable=%v", dir, portable)
	}
	// No data/ beside the test binary, so this falls through to the platform default.
	if dir, portable := resolveDataDir(""); portable {
		t.Fatalf("no data/ beside the exe, yet portable=true (%q)", dir)
	}
}
